...
public class JoyfulMomentController {
    ...
    private static final String PREF_NAME = "joyful_moment_prefs";
    ...
    public synchronized void startSession() {
        ...
        realtimeEngine = new JoyfulMomentRealtimeEngine(
                ...
                config,
                sessionDir,
                BuildConfig.SPEECHMATICS_API_KEY,
                BuildConfig.SPEECHMATICS_RT_URL,
    ...
    public synchronized void onAutoVideoSaved(String path) {
        if (path == null) {
        ...
        if (periodRecord != null) {
            periodRecord.videoPath = path;
            appendJson("period_log.jsonl", safeJson(periodRecord));
        }
        if (eventRecord != null) {
            eventRecord.videoPath = path;
            appendJson("event_log.jsonl", safeJson(eventRecord));
            eventStore.writeJson(new File(sessionDir, eventRecord.eventId + ".json"), safeJson(eventRecord));
        }
        appendAssetStatus("asset.auto_video.saved", "ok", path);
    }
    ...
    public synchronized void onAutoPhotoSaved(String path) {
        ...
        JoyfulMomentClusterer.PeriodRecord periodRecord = findPeriodById(lastTriggeredPeriodId);
        JoyfulMomentClusterer.EventRecord eventRecord = findEventById(lastTriggeredEventId);
        if (periodRecord != null && !periodRecord.photoPaths.contains(path)) {
            periodRecord.photoPaths.add(path);
            appendJson("period_log.jsonl", safeJson(periodRecord));
        }
        if (eventRecord != null && !eventRecord.photoPaths.contains(path)) {
            eventRecord.photoPaths.add(path);
            appendJson("event_log.jsonl", safeJson(eventRecord));
            eventStore.writeJson(new File(sessionDir, eventRecord.eventId + ".json"), safeJson(eventRecord));
        }
        appendAssetStatus("asset.auto_photo.saved", "ok", path);
    }
    ...
    private JoyfulMomentConfig loadConfig() {
    ...
    private void saveConfig(JoyfulMomentConfig config) {
        SharedPreferences prefs = context.getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
