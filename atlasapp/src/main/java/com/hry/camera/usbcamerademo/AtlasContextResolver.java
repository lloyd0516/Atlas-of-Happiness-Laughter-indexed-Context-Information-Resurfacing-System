...
public class AtlasContextResolver {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    for (int attempt = 1; attempt <= AppConfig.GPS_MAX_FIX_ATTEMPTS; attempt++) {
                        Location fresh = requestFreshLocation(manager);
                        best = chooseBetterLocation(best, fresh);
                        boolean accurateEnough = best != null && best.hasAccuracy()
                                && best.getAccuracy() <= AppConfig.GPS_DESIRED_ACCURACY_METERS;
                        if (accurateEnough || attempt == AppConfig.GPS_MAX_FIX_ATTEMPTS) {
                            break;
                        }
                        try {
                            Thread.sleep(AppConfig.GPS_FIX_RETRY_DELAY_MS);
    ...
    private static Location requestFreshLocation(final LocationManager manager) {
        ...
        try {
            ...
            latch.await(AppConfig.GPS_FRESH_FIX_TIMEOUT_MS, TimeUnit.MILLISECONDS);
        } catch (Exception ignored) {
        } finally {
            try {
